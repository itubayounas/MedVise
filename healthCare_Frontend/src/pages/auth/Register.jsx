...
export default function Register() {
  ...
  if (step === 1) return (
    <AuthShell title="Join MedVise" sub="Who are you registering as?">
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"20px" }}>
        {[
          { val:"patient", icon:"fa-user-injured", label:"Patient", desc:"Book appointments & track your health" },
          { val:"doctor",  icon:"fa-user-doctor",  label:"Doctor",  desc:"Manage patients & appointments" },
        ].map(r => (
          <button key={r.val} type="button" onClick={() => setRole(r.val)}
            style={{
              padding:"16px", borderRadius:"12px", textAlign:"left", cursor:"pointer",
              border: role === r.val ? "2px solid #426542" : "1.5px solid #e8dbc0",
              background: role === r.val ? "#f4f7f4" : "#fff",
              transition:"all .15s",
            }}>
            <i className={`fa-solid ${r.icon}`}
              style={{ fontSize:"22px", display:"block", marginBottom:"8px", color: role === r.val ? "#426542" : "#9ca3af" }}></i>
            <p style={{ fontWeight:600, fontSize:"15px", color:"#141814" }}>{r.label}</p>
            <p style={{ fontSize:"12px", color:"#6b7280", marginTop:"3px" }}>{r.desc}</p>
          </button>
        ))}
      </div>
      <button
        ...
        style={{
          ...
          background: role ? "#426542" : "#9ca3af", color:"#fff",
          fontSize:"14px", fontWeight:600, cursor: role ? "pointer" : "not-allowed",
      ...
      <p style={{ textAlign:"center", fontSize:"13px", color:"#6b7280", marginTop:"16px" }}>
        ...
        <Link to="/login" style={{ color:"#426542", fontWeight:600, textDecoration:"none" }}>Sign in</Link>
      </p>
    </AuthShell>
  );
  ...
  // ── Step 2: Basic info ──
  if (step === 2) return (
    <AuthShell title="Basic Information" sub={`Step 2 of ${role === "doctor" ? "3" : "2"}`}>
      <StepIndicator step={2} total={role === "doctor" ? 3 : 2} />
      <form onSubmit={role === "patient" ? handleSubmit : goToStep3}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
          ...
          <div style={{ gridColumn:"1 / -1" }}>
            ...
            <div style={{ position:"relative" }}>
              <input type={showPass ? "text" : "password"} className="input"
                placeholder="Min. 6 characters" style={{ paddingRight:"2.5rem" }}
          ...
          {role === "patient" && (
            <>
              <div>
                <label style={labelStyle}>
                  <i className="fa-solid fa-cake-candles" style={iconStyle}></i>Date of Birth
                </label>
              ...
              <div>
                <label style={labelStyle}>
                  <i className="fa-solid fa-droplet" style={iconStyle}></i>Blood Group
                </label>
          ...
          <div style={{ gridColumn:"1 / -1" }}>
            <label style={labelStyle}>
              <i className="fa-solid fa-location-dot" style={iconStyle}></i>City / Address
            </label>
            <input className="input" placeholder="Lahore, Pakistan"
  ...
  if (step === 3 && role === "doctor") {
      ...
      <AuthShell title="Application Submitted!" sub="">
        <div style={{ textAlign:"center", padding:"24px 0" }}>
          ...
          <p style={{ fontSize:"14px", color:"#6b7280", marginBottom:"20px", lineHeight:1.6 }}>
            Your doctor profile has been submitted. An admin will review and approve
            your account. You'll be able to log in after approval.
          </p>
          <Link to="/login" style={{ ...primaryBtnStyle, display:"inline-flex" }}>
    ...
    return (
      <AuthShell title="Doctor Profile" sub="Step 3 of 3 — Professional Details">
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"14px" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
            ...
            <div style={{ gridColumn:"1 / -1" }}>
              <label style={labelStyle}>
                <i className="fa-solid fa-stethoscope" style={iconStyle}></i>Specialty
              </label>
              <select className="input" value={form.specialty}
                onChange={e => set("specialty", e.target.value)} required>
            ...
            <div>
              <label style={labelStyle}>
                <i className="fa-solid fa-briefcase-medical" style={iconStyle}></i>Experience (yrs)
              </label>
              <input type="number" min="0" max="60" className="input" placeholder="5"
            ...
            <div>
              <label style={labelStyle}>
                <i className="fa-solid fa-dollar-sign" style={iconStyle}></i>Fee per Appointment ($)
              </label>
              <input type="number" min="0" className="input" placeholder="50"
            ...
            <div style={{ gridColumn:"1 / -1" }}>
              <label style={labelStyle}>
                <i className="fa-solid fa-pen-to-square" style={iconStyle}></i>Bio / About
              </label>
          ...
          <div>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"8px" }}>
              <label style={{ ...labelStyle, margin:0 }}>
                <i className="fa-solid fa-clock" style={iconStyle}></i>Availability
              </label>
              <button type="button" onClick={addSlot}
                style={{ fontSize:"13px", color:"#426542", background:"none", border:"none", cursor:"pointer", fontWeight:600 }}>
                <i className="fa-solid fa-plus" style={{ marginRight:"4px" }}></i>Add slot
              </button>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
              {slots.map((sl, i) => (
                <div key={i} style={{ display:"flex", gap:"8px", alignItems:"center", flexWrap:"wrap" }}>
                  <select className="input" style={{ flex:1, minWidth:"120px" }}
                    value={sl.day} onChange={e => updateSlot(i, "day", e.target.value)}>
                    {DAYS.map(d => <option key={d}>{d}</option>)}
                  </select>
                  <input type="time" className="input" style={{ width:"110px" }}
                    value={sl.start} onChange={e => updateSlot(i, "start", e.target.value)} />
                  <span style={{ fontSize:"13px", color:"#9ca3af" }}>to</span>
                  <input type="time" className="input" style={{ width:"110px" }}
                    value={sl.end} onChange={e => updateSlot(i, "end", e.target.value)} />
                  {slots.length > 1 && (
          ...
          {/* Approval notice */}
          <div style={{ padding:"10px 14px", background:"#fefce8", border:"0.5px solid #fde68a", borderRadius:"10px", display:"flex", alignItems:"flex-start", gap:"8px" }}>
            <i className="fa-solid fa-circle-info" style={{ color:"#d97706", marginTop:"1px", flexShrink:0 }}></i>
          ...
          <div style={{ display:"flex", gap:"10px" }}>
            <button type="button" onClick={() => setStep(2)} style={secondaryBtnStyle}>
              <i className="fa-solid fa-arrow-left"></i>Back
            </button>
...
const labelStyle = {
  ...
  fontSize:"13px", fontWeight:600, color:"#1e2420", marginBottom:"6px",
};
const iconStyle = { color:"#9ca3af", fontSize:"11px" };

const primaryBtnStyle = {
  padding:"9px 20px", borderRadius:"10px", border:"none",
  background:"#426542", color:"#fff", fontSize:"14px", fontWeight:600,
...
function AuthShell({ title, sub, children }) {
  return (
    <div style={{
      ...
      position:"relative",
      overflow:"hidden",
      background:"linear-gradient(145deg, #e8f5e8 0%, #f0f7f0 40%, #fdf6ed 100%)",
    }}>
      ...
      <div style={{
        ...
        borderRadius:"50%",
        background:"radial-gradient(circle, #779f77 0%, #558055 60%, transparent 100%)",
      ...
      <div style={{
        ...
        borderRadius:"50%",
        background:"radial-gradient(circle, #d4a853 0%, #c8956b 60%, transparent 100%)",
      ...
      <div style={{
        ...
        borderRadius:"50%",
        background:"radial-gradient(circle, #a5c1a5 0%, #779f77 70%, transparent 100%)",
        top:"35%", right:"5%",
        opacity:0.25,
        filter:"blur(50px)",
        pointerEvents:"none",
      ...
      <div style={{
        ...
        borderRadius:"50%",
        background:"radial-gradient(circle, #5ba4a4 0%, #3d7a7a 70%, transparent 100%)",
        top:"10%", right:"20%",
        opacity:0.2,
        filter:"blur(45px)",
        pointerEvents:"none",
      ...
      <div style={{ width:"100%", maxWidth:"520px", position:"relative", zIndex:10 }}>
        ...
        <div style={{
          ...
          padding:"28px",
          boxShadow:"0 20px 60px rgba(66,101,66,.15), 0 4px 16px rgba(0,0,0,.06)",
        }}>
          {children}
        </div>
      ...
      {Array.from({ length:total }, (_, i) => i + 1).map(n => (
        <div key={n} style={{ display:"flex", alignItems:"center", gap:"8px", flex:1 }}>
          <div style={{
            ...
            background: n < step ? "#426542" : n === step ? "#426542" : "#e8dbc0",
            color: n <= step ? "#fff" : "#9ca3af",
            boxShadow: n === step ? "0 0 0 4px rgba(66,101,66,.2)" : "none",
          }}>
            {n < step ? <i className="fa-solid fa-check" style={{ fontSize:"10px" }}></i> : n}
          </div>
          {n < total && (
            <div style={{
              height:"3px", flex:1, borderRadius:"2px", transition:"background .3s",
